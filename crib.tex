...
\begin{multicols}{2}
  ...
  \textbf{Theorem 3:}\\ $a \equiv b(\mod m) \iff a \mod  m = b \mod  m$\\
  \textbf{Theorem 4:}\\ Given $m \in \mathbb{Z}_{>0}$, integers $a \equiv b(\mod m) 
  ...
  \textbf{Theorem 5:}\\ Given $m \in \mathbb{Z}_{>0}$, if $a \equiv b(\mod m)$ and $c 
  \equiv d(\mod m)$, then $a + c = (b + d)(\mod m)$ and $ac = (bd)(\mod m)$\\
  \textbf{Corollary 2:}\\ Given $m \in \mathbb{Z}_{>0}$ and $a, b \in \mathbb{Z}$, $(a+b)\mod m 
  = (a\mod m + b\mod m)\mod m$ \textbf{and} $ab(\mod m) = 
  ...
  Given $b \in \mathbb{Z}$, $b > 1$, if $n \in \mathbb{Z}_{>0}$: $$n = a_kb^k + a_{k-1}b^{k-1} + ... + a_1b + a_0$$
